...
class SplitExpense(Base):
    ...
    __table_args__ = (
        UniqueConstraint("expense_id", name="uq_split_expenses_expense_id"),
    )
...
class SplitExpenseItem(Base):
    ...
    __table_args__ = (
        CheckConstraint("amount > 0", name="ck_split_expense_items_amount_positive"),
        CheckConstraint(
    ...
    split_expense_id: Mapped[uuid.UUID] = mapped_column(
        PGUUID(as_uuid=True),
        ForeignKey("split_expenses.id", ondelete="CASCADE"),
        nullable=False,
        index=True,
    )
