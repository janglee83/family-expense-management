...
`background`/`foreground`/`card`/`muted`/`border` also move from the current cool hue (~255-262) to a warm-neutral hue (~40-60) in **both** dark blocks, keeping each token's existing lightness/chroma magnitude — only the hue angle shifts, so the light/dark contrast relationships that already work don't need to be re-derived from scratch. Exact final OKLCH numbers are tuned during implementation (verify WCAG AA for text-on-primary and primary-on-background at each step); the table above is the starting point, not a pixel-locked spec.
